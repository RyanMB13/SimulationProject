...
public class DirectMappedCache extends JFrame {
    ...
    private void setupAnimationTimer() {
        animationTimer = new Timer(200, e -> {
            if (!memoryQueue.isEmpty()) {
                ...
                // Update stats chart periodically
    ...
    private void updateCacheDisplay(int index, boolean valid, int memoryBlockNumber) {
        ...
        if (valid) {
            cacheBlockPanels[index].setBackground(new Color(240, 249, 255)); // Light blue for valid
        } else {
            cacheBlockPanels[index].setBackground(EMPTY_CACHE_COLOR); // Gray for empty
        }
    }
    ...
    private void prepareTest(int testType) {
        ...
        // Store total memory accesses for progress calculation
